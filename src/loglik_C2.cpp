...
//#pragma omp parallel for private(i, j, l, k, gauss_quad, u, u1, u2, w, w1, w2, sq, rep1, rep2, uu1, uu2, ww1, ww2, tmp1, tmp2, tmp3, tmp5, tmp7, tmp8) num_threads(4) reduction(+: rr)
  for(i=0; i<nf; i++){
    ...
    if(nr==2){
      ...
      tmp8 +=  -log(hf(vppc(tmp, lam01, cut_F, 0.0, 0.0), tmpdel,  rho, copula)*ppc(exam_age[1], LAM03[1], cut[1], 0.0, 0.0)
                      + sum(w*vdcopf(vpx, vppc(u, lam01, cut_F, 0.0, 0.0), rho, 0.0, copula)*vdpc(u, lam01, cut_F, 0.0)*vppc(u, LAM03[1], cut[1], 0.0, 0.0)*ppc(exam_age[1], LAM12[1], cut[1], 0.0, 0.0)/vppc(u, LAM12[1], cut[1], 0.0, 0.0)));
      ...
      double F0 =  ppc(X[0], lam01, cut_F, 0.0, 0.0);

      for(j=1; j<nr; j++){
        Sf(j-1, 0)  = SAf(j-1, 0) = F0;
        Sf(j-1, 1)  = ppc(X[j], lam01, cut_F, 0.0, 0.0);
        SAf(j-1, 1)  = ppc(exam_age[j], lam01, cut_F, 0.0, 0.0);
        Sc[j-1]  = h1(F0, ppc(X[j], lam01, cut_F, 0.0, 0.0), rho, copula);
        SAc[j-1]  = h1(F0, ppc(exam_age[j], lam01, cut_F, 0.0, 0.0), rho, copula);
      }
      ...
      NumericVector vpx = rep(F0, 20);
      NumericVector vpx2 = rep(F0, 20*20);
      ...
      for(j=0; j<pair_nr; j++){
        ...
        tmp8 -= log(pccopf(SA(j,_), F0, rho, copula)*ppc(exam_age[j1], LAM03[j1], cut[j1], 0.0, 0.0)*ppc(exam_age[j2], LAM03[j2], cut[j2], 0.0, 0.0)
                      + sum(w2*vhc1(vh1(vpx, vppc(u2, lam01, cut_F, 0.0, 0.0), rho, copula), rep(SA(j,0), 20), vpx, rho, copula)
                              *vdcopf(vpx, vppc(u2, lam01, cut_F, 0.0, 0.0), rho, 0.0, copula)*vdpc(u2, lam01, cut_F, 0.0)*vppc(u2, LAM03[j2], cut2, 0.0, 0.0)
                              /vppc(u2, LAM12[j2], cut2, 0.0, 0.0))*ppc(exam_age[j2],LAM12[j2],  cut2, 0.0, 0.0)*ppc(exam_age[j1], LAM03[j1], cut1, 0.0, 0.0)
                              + sum(w1*vhc1(vh1(vpx, vppc(u1, lam01, cut_F, 0.0, 0.0), rho, copula), rep(SA(j,1), 20), vpx, rho, copula)
                              *vdcopf(vpx, vppc(u1, lam01, cut_F, 0.0, 0.0), rho, 0.0, copula)*vdpc(u1, lam01, cut_F, 0.0)*vppc(u1, LAM03[j1], cut1, 0.0, 0.0)
                              /vppc(u1, LAM12[j1], cut1, 0.0, 0.0))*ppc(exam_age[j1],  LAM12[j1], cut1, 0.0, 0.0)*ppc(exam_age[j2], LAM03[j2], cut2, 0.0, 0.0)
                              +sum(ww1*ww2*vdccopf(vh1(vpx2, vppc(uu1, lam01, cut_F, 0.0, 0.0), rho, copula) , vh1(vpx2, vppc(uu2, lam01, cut_F, 0.0, 0.0), rho, copula),  vpx2, rho, 0.0, copula)
                              *vdcopf(vpx2, vppc(uu1, lam01, cut_F, 0.0, 0.0), rho, 0.0, copula)*vdcopf(vpx2, vppc(uu2, lam01, cut_F, 0.0, 0.0), rho, 0.0, copula)
 ...
 // double theta = exp(par[1]);
  ...
  for(i=0; i<nf; i++){
    ...
    if(nr==2){
      ...
      if(sum(del2)==2) tmp1 += dcopf(S0, rho, 1.0, copula);
      else tmp1 += log(hf(S0, del2,  rho, copula));
      ...
      tmp8 +=  -log(h1(tmpS1[0], tmpS1[1], rho, copula)*ppc(exam_age[1], LAM03[1], cut[1], 0.0, 0.0)*pG0(rid, tmpIG1, p)/pg0
                  + h1(tmpS2[0], tmpS2[1], rho, copula)*ppc(exam_age[1], LAM03[1], cut[1], 0.0, 0.0)*pG0(rid, tmpIG2, p)/pg0
                  + sum(w*vdcopf(vpx, vppc(u, lam01*exp(alpha*0), cut_F, 0.0, 0.0), rho, 0.0, copula)*vdpc(u, lam01*exp(alpha*0), cut_F, 0.0)*vppc(u, LAM03[1], cut[1], 0.0, 0.0)*ppc(exam_age[1], LAM12[1], cut[1], 0.0, 0.0)/vppc(u, LAM12[1], cut[1], 0.0, 0.0))*pG0(rid, tmpIG1, p)/pg0
                  + sum(w*vdcopf(vpx, vppc(u, lam01*exp(alpha*1), cut_F, 0.0, 0.0), rho, 0.0, copula)*vdpc(u, lam01*exp(alpha*1), cut_F, 0.0)*vppc(u, LAM03[1], cut[1], 0.0, 0.0)*ppc(exam_age[1], LAM12[1], cut[1], 0.0, 0.0)/vppc(u, LAM12[1], cut[1], 0.0, 0.0))*pG0(rid, tmpIG2, p)/pg0);
      ...
      for(j=1; j<nr; j++){
        Sf(j-1, 0)  = SAf(j-1, 0) = px;
        Sf(j-1, 1)  = ppc(X[j], lam01*exp(alpha*IG[j]), cut_F, 0.0, 0.0);
        SAf(j-1, 1)  = ppc(exam_age[j], lam01*exp(alpha*IG[j]), cut_F, 0.0, 0.0);
        Sc[j-1]  = h1(px, ppc(X[j], lam01*exp(alpha*IG[j]), cut_F, 0.0, 0.0), rho, copula);
        SAc[j-1]  = h1(px, ppc(exam_age[j], lam01*exp(alpha*IG[j]), cut_F, 0.0, 0.0), rho, copula);
        SAg0[j-1]  = h1(px, ppc(exam_age[j], lam01*exp(alpha*0), cut_F, 0.0, 0.0), rho, copula);
        SAg1[j-1]  = h1(px, ppc(exam_age[j], lam01*exp(alpha*1), cut_F, 0.0, 0.0), rho, copula);
      ...
      for(j=0; j<pair_nr; j++){
        ...
        if (sum(fam_del2(j,_))==2) {
          tmp11 = dccopf(S(j,_),  px, rho, 1.0, copula) + dcopf(Sf(comb(0,j)-1,_), rho, 1.0, copula) + dcopf(Sf(comb(1,j)-1,_), rho, 1.0, copula);
        }  else if(sum(fam_del2(j,_))==1) {
          tmp11 = log(hcf(S(j,_), fam_del2(j,_), px, rho, copula)) + fam_del2(j,0)*dcopf(Sf(comb(0,j)-1,_), rho, 1.0, copula) + fam_del2(j,1)*dcopf(Sf(comb(1,j)-1,_), rho, 1.0, copula);
        } else {
          tmp11 =  log(pccopf(S(j,_), px, rho, copula));
        }
