...
pair.logL <-	function( par, Y.fam, X.fam,  Y.proband, X.proband, Y.R, X.R, Y.S,
                        outdata.F0 = NULL, outdata.proband = NULL, cut, R.fR, A.fR, cut.R, R.fS, A.fS, cut.S = NULL,  G, Age, Cal, design, full, no.death, copula){
  ...
  if(is.null(G)) res <- loglikFD2_pch(par, theta, Y.fam, X.fam, as.matrix(Y.proband), as.matrix(X.proband),  Age, Cal, cut, lam03, gauleg.f, utils::combn, copula)
  else res <- loglikFD2_pch_gene(par, theta, Y.fam, X.fam, as.matrix(Y.proband), as.matrix(X.proband),  Age, Cal, cut, lam03, gauleg.f, utils::combn, copula)
  ...
  nf <- length(Y.fam)
  for(i in 1:nf){
    res1 = res1 +  loglikFD2_pch_gene_R(par, Y_F = Y.fam[i], X_F = X.fam[i], Y_proband = t(as.matrix(Y.proband[i,])), X_proband = t(as.matrix(data.proband[i,])),
...
loglikFD2_pch_R <- function(par, Y_F, X_F,  Y_proband, X_proband,
  ...
  res <- loglikFD2_pch(par, theta, Y_F, X_F, Y_proband, X_proband,  Age, Cal, cut_F, lam03, fgau, combn, copula)
  return(res)
}
...
loglikFD2_pch_gene_R <- function(par, Y_F, X_F,  Y_proband, X_proband,
  ...
  res <- loglikFD2_pch_gene(par, theta, Y_F, X_F, Y_proband, X_proband,  Age, Cal, cut_F, lam03, fgau, combn, copula)
  return(res)
}
